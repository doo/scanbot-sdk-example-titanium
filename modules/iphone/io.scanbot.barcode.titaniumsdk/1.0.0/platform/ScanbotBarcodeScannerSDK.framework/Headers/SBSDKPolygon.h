 ...
 * Compares the receiver to another instance of SBSDKPolygon.
 * @param polygon The polygon instance the receiver is compared to.
 * @return YES, if the points of polygon are similar to the points of the receiver, NO otherwise.
...
/**
 Calculates the estimated size of the perspective-corrected and cropped image when applying the receiver to
 an image with given size.
 
 @param size The size of the original image the receiver should be applied to.
 @param imageScale The scaling factor of the image.
 @param focalLength The focal length of the lens the image was taken with in millimeters.
 @param sensorWidth The width of the sensor the image was taken with in millimeters.
 
 @return The estimated size of the cropped and perspective-corrected image, when the receiver was applied.
 */
 ...
 * @param orientation Is currently used camera - front camera. Used to calculate starting point.
 * @return An UIBezierPath instance representing the polygon.
 */
- (UIBezierPath *)bezierPathWithCornerRadius:(CGFloat)cornerRadius
                             forPreviewLayer:(AVCaptureVideoPreviewLayer *)previewLayer
                                 orientation:(UIDeviceOrientation)orientation isFrontCamera:(BOOL)isFrontCamera;
#endif
